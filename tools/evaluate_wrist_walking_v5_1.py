#!/usr/bin/env python3
"""Independent evaluation utility for a saved SafeBand walking model."""
...
def main():
    ...
    ap.add_argument("--input",required=True); ap.add_argument("--model",required=True); ap.add_argument("--out",required=True)
    args=ap.parse_args()
    df=pd.read_csv(args.input); bundle=joblib.load(args.model)
    feats=bundle["feature_names"]; missing=[c for c in feats if c not in df.columns]
    if missing: raise ValueError(f"Missing model features: {missing}")
    pred=bundle["model"].predict(df[feats].to_numpy(float))
    cal=bundle.get("calibration",{})
    if cal.get("enabled"): pred=float(cal["slope"])*pred+float(cal["intercept"])
    y=df.hr_bpm.to_numpy(float)
    out=df[["subject","activity","record","start_sample","start_time","hr_bpm"]].copy()
    out["prediction_bpm"]=pred; out["error_bpm"]=pred-y; out["abs_error_bpm"]=np.abs(pred-y)
    def grouped(col): return {str(k):scores(g.hr_bpm,g.prediction_bpm) for k,g in out.groupby(col,sort=True)}
    by_subject=grouped("subject"); by_record=grouped("record")
    worst_subject=max(by_subject,key=lambda k:by_subject[k]["mae_bpm"])
    worst_record=max(by_record,key=lambda k:by_record[k]["mae_bpm"])
    report={"version":"PPG WALKING V5.1 EVALUATION","model":args.model,"overall":scores(y,pred),
            "by_subject":by_subject,"by_record":by_record,
            "worst_subject":worst_subject,"worst_record":worst_record}
