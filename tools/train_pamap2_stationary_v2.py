...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="PAMAP2 stationary/postural RBF-SVM benchmark."
    )
    ...
    if not data_path.exists():
        raise FileNotFoundError(
            f"Dataset not found:\n  {data_path}\n"
            f"Run this command from the repository root or pass --data explicitly."
        )
    ...
    print(f"Class counts    : {np.bincount(y, minlength=3).tolist()}")
    ...
    np.save(out_dir / "stationary_v2_features.npy", F)
    ...
    for fold, (tr, te) in enumerate(cv.split(F, y, groups), start=1):
        print(f"\n===== OUTER FOLD {fold}/5 =====")
        C, gamma, inner_score = select_params(
            F[tr], y[tr], groups[tr], args.seed + fold
        )

        print(f"Selected C={C}, gamma={gamma}, inner macro-F1={inner_score:.4f}")
    ...
    pooled_true = np.asarray(pooled_true, dtype=np.int64)
    pooled_pred = np.asarray(pooled_pred, dtype=np.int64)
    pooled = evaluate(pooled_true, pooled_pred)
    pooled.update(explicit_class_metrics(pooled_true, pooled_pred))

    # After the unbiased outer benchmark, select a deployment candidate using
    # inner grouped CV over the complete dataset. This does NOT change the
    # reported outer-test metrics.
    deploy_C, deploy_gamma, deploy_inner = select_params(
        F, y, groups, args.seed + 1000
    )
    ...
    deployment_model.fit(F, y)
    deployment_path = out_dir / "pamap2_stationary_v2_deployment.joblib"
    joblib.dump(deployment_model, deployment_path)

    report = {
        "experiment": "pamap2_stationary_accgyro_v2",
        "model": "rbf_svm_window_features",
        "data": str(data_path),
        "seed": args.seed,
        "samples": int(len(y)),
        ...
        "validation": {
            ...
            "inner": "4-fold StratifiedGroupKFold inside each outer training fold",
            "selection_metric": "macro_f1",
            "scaling": "StandardScaler fitted only on outer training data",
        ...
        "deployment_candidate": {
            "C": deploy_C,
            "gamma": deploy_gamma,
            "inner_macro_f1": deploy_inner,
            "model_path": str(deployment_path),
            "note": "Fit on all available windows after the outer benchmark; not an independent test model.",
        },
    }

    report_path = out_dir / "pamap2_stationary_v2_report.json"
    ...
    print("\n===== POOLED OUTER RESULT =====")
    print(f"Accuracy          : {pooled['accuracy']:.4%}")
    print(f"Balanced accuracy : {pooled['balanced_accuracy']:.4%}")
    print(f"Macro F1          : {pooled['macro_f1']:.4%}")
    print("Class F1          :", [
        f"{CLASS_NAMES[i]}={pooled['per_class_f1'][i]:.4f}"
        for i in range(3)
    ])
    print("Confusion matrix:")
    print(np.asarray(pooled["confusion_matrix"]))
    print(f"Deployment candidate: C={deploy_C}, gamma={deploy_gamma}, inner macro-F1={deploy_inner:.4f}")
